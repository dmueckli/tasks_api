...
try { // Try to connect to databases
...
try { // Try to get user credentials from db using the given accesstoken
    ...
    if (strtotime($returned_accesstokenexpiry) < time()) {
        ...
        $response->setHttpStatusCode(401);
        $response->setSuccess(false);
        $response->addMessage('Access token has been expired.');
        $response->send();
        exit();
    }
} catch (PDOException $ex) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        try {
            ...
            if ($title_updated === false && $description_updated === false && $deadline_updated === false && $completed_updated === false) {
                ...
                $response->setHttpStatusCode(400);
                $response->setSuccess(false);
                $response->addMessage('No task fields provided to update.');
                $response->send();
                exit;
            }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        try {
            ...
            if ($rowCount === 0) {
                ...
                $response->setHttpStatusCode(500);
                $response->setSuccess(false);
                $response->addMessage('Failed to retrieve task from database after creation.');
                $response->send();
                exit;
            }
            ...
            $response->setHttpStatusCode(500);
            $response->setSuccess(false);
            $response->addMessage('Failed to insert task into database - please check the submitted data.');
            $response->send();
            exit();
        }
    } else {
